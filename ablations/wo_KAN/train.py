...
def main(args: argparse.Namespace) -> None:
    ...
    for epoch in range(n_epochs):
        ...
        if dev_avg_acc['AttackAttr_acc'] > best_acc:
            best_acc = dev_avg_acc['AttackAttr_acc']
...
def plot(history, exp_name):
    ...
    # -------- Plot LOSS --------
    fig_loss, axes_loss = plt.subplots(nrows=2, ncols=4, figsize=(40, 8))
    ...
    # -------- Plot ACCURACY --------
    fig_acc, axes_acc = plt.subplots(nrows=2, ncols=4, figsize=(40, 8))
...
def train_phase(train_loader=None, model=None, freeze_backbone=False, optimizer=None, device=None):
    ...
    for waveform, ground_truth in train_loader:
        ...
        running_loss = multi_task_losses.mean() # total loss for 1 batch
    ...
    avg_loss = {
        **{task_name: loss_value / total_spoof_samples for task_name, loss_value in total_loss.items()}
    }
