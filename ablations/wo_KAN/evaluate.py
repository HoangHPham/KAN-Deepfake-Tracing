...
def eval_phase(loader, model, device):
    ...
    avg_acc = {
        **{task_name: acc / total_spoof_samples for task_name, acc in total_acc.items() if task_name != "AttackAttr_acc"}
    }
    ...
    balanced_acc["AttackAttr_bal_acc"] = sum(balanced_acc[t] for t in balanced_acc.keys()) / 7
