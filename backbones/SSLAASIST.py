...
class SSLModel(nn.Module):
    def __init__(
        ...
        cp_path='./backbones/pretrained_weights/xlsr2_300m.pt',
    ):
        ...
        if use_pretrained:
            ...
            # Load pretrained model to extract configuration
            task_arg = argparse.Namespace(task='audio_pretraining')
            task = fairseq.tasks.setup_task(task_arg)
            _, cfg, task = fairseq.checkpoint_utils.load_model_ensemble_and_task([cp_path], task=task)

            # Create a fresh model with the same config
            self.model = task.build_model(cfg.model, from_checkpoint=False)
        ...
        self.out_dim = 1024  # XLSR2_300m hidden size
    ...
    def _init_weights(self):
        ...
        # Special initialization for certain components
...
#         cp_path = './backbones/pretrained_weights/xlsr2_300m.pt'   # Change the pre-trained XLSR model path. 
...
class SSLAASIST(nn.Module):
    def __init__(self, device, use_pretrained_ssl=False):
        super().__init__()
        self.device = device
        ...
        ####
        self.ssl_model = SSLModel(self.device, use_pretrained=use_pretrained_ssl)
...
class SSLAASISTBackbone(nn.Module):
    def __init__(self, base_model: SSLAASIST, freeze: bool = False):
        super().__init__()
    ...
    def forward(self, x, **kwargs):
        last_hidden, _ = self.base(x, **kwargs)
        return last_hidden 